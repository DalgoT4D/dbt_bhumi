WITH normalized_students AS (
    ...
    FROM {{ ref('All_Students_STP') }} AS s
),
...
session_students AS (
    ...
    CROSS JOIN
        LATERAL (
            SELECT
                unnest(coalesce(sa.present_students_list_arr, ARRAY[]::text[])) AS student_name_raw,
                'Present'::text AS status
            UNION ALL
            SELECT
                unnest(coalesce(sa.absent_students_list_arr, ARRAY[]::text[])) AS student_name_raw,
...
FROM session_students AS ss
LEFT JOIN normalized_students AS st
    ON
        ss.norm_school = st.norm_school
