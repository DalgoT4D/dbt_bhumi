...
with master as (
    ...
    select
        ...
        case when btrim("Actual_Duration__days_") ~ '^\d+$' then btrim("Actual_Duration__days_")::integer else 0 end as actual_duration_days,
        case when btrim("Planned_Duration__days_") ~ '^\d+$' then btrim("Planned_Duration__days_")::integer else 0 end as planned_duration_days,
        case when btrim("Date_of_Confirmation_from_CR_team") ~ '^\d+$' then btrim("Date_of_Confirmation_from_CR_team")::integer else 0 end as cr_confirmation_days,
        case when btrim("Date_of_funds_received") ~ '^\d+$' then btrim("Date_of_funds_received")::integer else 0 end as funds_received_days,
        case when btrim("No_of_Washroom_Constructred__Project_details_") ~ '^\d+$' then btrim("No_of_Washroom_Constructred__Project_details_")::integer else 0 end as washroom_constructed,
        case when btrim("No_of_Washroom_Renovated___Project_details_") ~ '^\d+$' then btrim("No_of_Washroom_Renovated___Project_details_")::integer else 0 end as washroom_renovated,
        case when btrim("No_of_Classroom_Constructred___Project_details_") ~ '^\d+$' then btrim("No_of_Classroom_Constructred___Project_details_")::integer else 0 end as classroom_constructed,
        case when btrim("No_of_Classroom_Renovated___Project_details_") ~ '^\d+$' then btrim("No_of_Classroom_Renovated___Project_details_")::integer else 0 end as classroom_renovated,
        case when btrim("No_of_furnitures_provided___Project_details_") ~ '^\d+$' then btrim("No_of_furnitures_provided___Project_details_")::integer else 0 end as furniture_count,
        case when btrim("No_of_RO_Plant_Installed___Project_details_") ~ '^\d+$' then btrim("No_of_RO_Plant_Installed___Project_details_")::integer else 0 end as ro_plants,
        case when btrim("No_of_existing_RO_Plant_AMC_extened___Project_details_") ~ '^\d+$' then btrim("No_of_existing_RO_Plant_AMC_extened___Project_details_")::integer else 0 end as ro_plant_amc_extended,
        case when btrim("No_of_Solar_panel_work_Installed___Project_details_") ~ '^\d+$' then btrim("No_of_Solar_panel_work_Installed___Project_details_")::integer else 0 end as solar_panels,
        case when btrim("Others___Project_details_") ~ '^\d+$' then btrim("Others___Project_details_")::integer else 0 end as other_project_counts,
        -- TEXT REMAINING
