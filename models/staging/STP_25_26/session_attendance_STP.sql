...
SELECT
    ...
    CASE WHEN trim(src."Present_Students"::text) ~ '^\d+$' THEN trim(src."Present_Students"::text)::int END AS present_students,
    CASE WHEN trim(src."Absent_Students"::text) ~ '^\d+$' THEN trim(src."Absent_Students"::text)::int END AS absent_students,
    CASE WHEN trim(src."Total_Student"::text) ~ '^\d+$' THEN trim(src."Total_Student"::text)::int END AS total_students,
    ...
    CASE
        WHEN trim(src."Volunteering_hrs_Engaged"::text) ~ '^[\\d\\.,]+$'
            THEN
