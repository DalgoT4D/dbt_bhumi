...
with accounts as (
    select
        ...
        case
            when regexp_replace(btrim("Qty"::text), '[^0-9.-]', '', 'g') ~ '^-?[0-9]+(\.[0-9]+)?$'
                then regexp_replace(btrim("Qty"::text), '[^0-9.-]', '', 'g')::numeric
        end as qty,
        ...
        case
            when regexp_replace(btrim("Amount"::text), '[^0-9.-]', '', 'g') ~ '^-?[0-9]+(\.[0-9]+)?$'
                then regexp_replace(btrim("Amount"::text), '[^0-9.-]', '', 'g')::numeric
        end as amount,
        coalesce(("Status"::jsonb)->>'zc_display_value', ("Status"::jsonb)->>'Name', nullif(btrim("Status"::text), '')) as status,
        case
            when regexp_replace(btrim("Discount"::text), '[^0-9.-]', '', 'g') ~ '^-?[0-9]+(\.[0-9]+)?$'
                then regexp_replace(btrim("Discount"::text), '[^0-9.-]', '', 'g')::numeric
        end as discount,
        ...
        case
            when regexp_replace(btrim("Net_Amount"::text), '[^0-9.-]', '', 'g') ~ '^-?[0-9]+(\.[0-9]+)?$'
                then regexp_replace(btrim("Net_Amount"::text), '[^0-9.-]', '', 'g')::numeric
        end as net_amount,
