...
with assessment_completion_baseline as (
    ...
    from 
        {{ ref('base_mid_end_comb_scores_26_27_fct') }} as f
    inner join {{ ref('base_mid_end_comb_students_26_27_dim') }} as d
...
--         count(distinct case when f.rc_level_endline_end in ('','Not Assessed') then f.student_id end) as unassessed_students_rc_end,
--         count(distinct case when f.rf_level_endline_end in ('','Not Assessed') then f.student_id end) as unassessed_students_rf_end,
--         count(distinct case when f.math_level_endline_end in ('','Not Assessed') then f.student_id end) as unassessed_students_math_end
--     from 
--         {{ ref('base_mid_end_comb_scores_26_27_fct') }} as f
--     inner join {{ ref('base_mid_end_comb_students_26_27_dim') }} as d
--         on f.student_id = d.student_id
--     where d.endline_attendence = True
--     group by d.city_end, d.student_grade_end
-- ),
